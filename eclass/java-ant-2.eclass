...
# Changes:
#   May 2007:
#     I make bsfix make one pass for all things and add some glocal targets for
...
#                                directories
#	JAVA_ANT_BSFIX_EXTRA_ARGS: be carefull by setting some args there, use at your own risks
#
	...
	if [ -z "${want_source}" -o -z "${want_target}" ]; then
		...
		if [[ "${using_new}" ]]; then
			...
			if [[ ! -f ${rewriter3} ]]; then
				...
				if [[ -n ${JAVA_ANT_BSFIX_EXTRA_ARGS} ]]; then
