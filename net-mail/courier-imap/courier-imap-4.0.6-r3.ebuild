...
src_unpack() {
	...
	eautoreconf || die "eautoreconf on . failed"
	eend $?
	for i in numlib md5 sha1 \
			   bdbobj gdbmobj soxwrap \
			   unicode rfc822/ random128 \
			   rfc1035 rfc2045 liblock \
			   maildir makedat libhmac \
			   waitlib tcpd imap;do

		cd "${S}/$i"
		ebegin "Recreating $i/configure"
		eautoreconf || die "eautoreconf $i failed"
		eend $?
	done
}
